// many many thanks to
// iq https://www.shadertoy.com/user/iq
// foxes https://www.shadertoy.com/view/4lfSzS
//
...
void main()
{
    ...
    s2=min(1.0,s2*2.2);
    gl_FragColor = vec4( mix(vec3(1.0,1.0,1.0),vec3(1.0),pow(s1,60.0))*s1, 1.0 );
    gl_FragColor += vec4( mix(mix(vec3(0.0,0.0,0.0),vec3(0.0,0.0,0.0),pow(s2,2.0)),vec3(1.0),pow(s2,10.0))*s2, 1.0 );
    ...
    if (zero>0.0) {
    	...
    	s4.x=pow(s4.x,3.0);
    	gl_FragColor.xyz += mix(mix(vec3(1.0,1.0,1.0),vec3(1.0,1.0,1.0),s4.y*1.9),vec3(1.0,1.0,1.0),s4.w*0.75)*s4.x*pow(s4.z*2.5,3.0)*0.2*zero;
    ...
    // distance
    float d = min(sdRing(p, cs, ra, th),sdRing(-p, cs, ra, th));
    
    gl_FragColor = max( vec4(0.0), gl_FragColor );
	gl_FragColor = min( vec4(1.0), gl_FragColor );

    vec3 col = (d > 0.0) ? gl_FragColor.xyz : vec3(1.0,1.0,1.0);
    gl_FragColor.xyz = mix( col, gl_FragColor.xyz, 1.0-smoothstep(0.0, 0.006, abs(d)) );
}
